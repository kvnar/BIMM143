...
We can use the datapasta package package to scrape 
```{r}
cdc <- data.frame(
...
We can use the `lubridate` package to ease the pain of working with dates
...
One more `inner_join()` to add all our metadata in `meta` on to our `abdata` table:
...
Why are certain antigens and not other very variable in their detected levels here?
